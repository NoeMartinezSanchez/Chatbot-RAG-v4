...
- `fetch_mongodb_interactions()` maps each `conversations` doc to interaction dicts pairing `user` msg → next `assistant` msg; `fuentes_usadas` read from `sources_used[].metadata.source_file`; `tokens_used` from message tokens (or `total_tokens`); timestamps normalized to UTC via `_ensure_utc()` before `.isoformat()` (avoid naive/aware serialization errors)
